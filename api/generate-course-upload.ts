...
export default async function handler(req: RequestLike, res: ResponseLike) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({ model: 'gemini-2.5-flash', contents: [{ role: 'user', parts: [{ inlineData: { mimeType, data: req.body.toString('base64') } }, { text: prompt }] }], config: { responseMimeType: 'application/json', temperature: 0.3 } });
