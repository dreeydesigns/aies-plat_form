...
const MAX_DOCUMENT_BYTES = 3_500_000;
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (!document?.base64 || !document?.mimeType || !document?.name || !brief) {
  ...
  if (Buffer.byteLength(document.base64, 'base64') > MAX_DOCUMENT_BYTES) {
    return res.status(413).json({ error: 'The document is too large. Upload a file smaller than 3.5 MB.' });
  }
